	...
	for (int i = 0; i < NUM_ROWS_OF_INVADERS; i++)
	{
		for (int j = 0; j < NUM_INVADERS_PER_ROW; j++)
		{
			if (m_EnemyShip[i][j].Update(deltaTime))
			{
				...
				pMissile->Initialise(Vector2(m_EnemyShip[i][j].GetPosition().x + (m_EnemyShip[i][j].GetSize().x / 2.0f), m_EnemyShip[i][j].GetPosition().y + 1), Vector2(-MISSILE_SPEED, 0));
	...
	if (m_MovingDown)
	{
		for (int i = 0; i < NUM_ROWS_OF_INVADERS; i++)
		{
			for (int j = NUM_INVADERS_PER_ROW - 1; j >= 0; j--)
			{
				...
				{
					if ((m_EnemyShip[i][j].GetPosition().x + (m_EnemyShip[i][j].GetSize().x)) >= SCREEN_HEIGHT)
					{
